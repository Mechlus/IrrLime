...
void CCameraSceneNode::render()
{
	...
	if (TargetAndRotationAreBound)
	{
		...
		// If upvector and vector to the target are the same, we have a problem. Solve it:
		const f32 dp = tgtv.dotProduct(up);
		...
		// Use LookAt matrix when bound to target
		ViewArea.getTransform(video::ETS_VIEW).buildCameraLookAtMatrixLH(pos, Target, up);
	}
	else
	{
		// Use free rotation mode when target is not bound
		ViewArea.getTransform(video::ETS_VIEW).buildCameraRotationMatrix(pos, getAbsoluteTransformation().getRotationDegrees(), up);
	}
