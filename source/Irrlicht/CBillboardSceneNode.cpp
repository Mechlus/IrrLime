...
void CBillboardSceneNode::render()
{
	...
	pos.Y += yPivot; // apply vertical pivot
...
void CBillboardSceneNode::setPivotY(f32 pivot)
{
	yPivot = pivot;
}

f32 CBillboardSceneNode::getPivotY() const
{
	return yPivot;
}

void CBillboardSceneNode::setRotLock(const core::vector3d<bool>& rots)
{
	rotLock = rots;
}

core::vector3d<bool> CBillboardSceneNode::getRotLock() const
{
	return rotLock;
}
...
ISceneNode* CBillboardSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
	...
	nb->rotLock = this->rotLock;
	nb->yPivot = this->yPivot;
