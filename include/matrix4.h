...
namespace irr
{
	namespace core
	{
		...
		public:
			...
			//! Builds a matrix based on euler rotation. // TEMP
			CMatrix4<T>& buildCameraRotationMatrix(
				const vector3df& position,
		...
		*/
		...
		template <class T>
		inline CMatrix4<T>& CMatrix4<T>::buildCameraRotationMatrix(
			const vector3df& position,
			const vector3df& rotation,
			const vector3df& upVector)
		{
			// Convert rotation from degrees to radians
			f32 pitch = core::degToRad(rotation.X);
			f32 yaw = core::degToRad(rotation.Y + 180.0);
			f32 roll = core::degToRad(rotation.Z + 180.0);

			// Calculate forward (Z-axis) using pitch and yaw
			vector3df zaxis;
			zaxis.X = -sin(yaw) * cos(pitch);  // Inverted +Z forward in left-handed system
			zaxis.Y = -sin(pitch);
			zaxis.Z = -cos(yaw) * cos(pitch);  // Inverted to point inside
			zaxis.normalize();

			// Calculate right (X-axis) from yaw, pitch, and roll
			vector3df xaxis;
			xaxis.X = cos(yaw) * cos(roll) + sin(yaw) * sin(pitch) * sin(roll);
			xaxis.Y = cos(pitch) * sin(roll);
			xaxis.Z = -sin(yaw) * cos(roll) + cos(yaw) * sin(pitch) * sin(roll); // Adjusted for inverted system
			xaxis.normalize();

			// Calculate up (Y-axis) by cross-product of forward (zaxis) and right (xaxis)
			vector3df yaxis = zaxis.crossProduct(xaxis); // Cross to get Y-axis
			yaxis.normalize();

			// Construct 4x4 matrix for the camera
			M[0] = (T)xaxis.X;
			M[1] = (T)yaxis.X;
			M[2] = (T)zaxis.X;
			M[3] = 0;
